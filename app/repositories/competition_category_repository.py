...
class CompetitionCategoryRepository:
    ...
    def get_competition_category(self, competition_category_id: int) -> Type[CompetitionCategory] | None:
        return self.db.query(CompetitionCategory).filter(CompetitionCategory.ID_competition_category == competition_category.category_id).first()
    ...
    def create_competition_category(self, create_competition_category: schemas.CompetitionCategoryCreate):
        db_category = models.CompetitionCategory(**category.dict())
        db.add(db_category)
        db.commit()
    ...
    def delete_competition_category(self, competition_category_id: int) -> Type[CompetitionCategory] | None:
        db_category = self.get_competition_category(category_id)
        if db_category:
